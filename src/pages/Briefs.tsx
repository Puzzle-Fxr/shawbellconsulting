    ...
    <div className="page-enter">
      ...
      <section className="bg-white py-20 lg:py-28">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          >
            <div className="grid lg:grid-cols-2 gap-8 bg-gradient-to-br from-steel/5 to-ocean/5 rounded-2xl border border-platinum p-8 lg:p-12">
              <div>
                ...
                <h2 className="font-heading font-bold text-2xl md:text-3xl text-steel mb-4">{briefs[0].title}</h2>
                <p className="text-gray-600 leading-relaxed mb-6">{briefs[0].excerpt}</p>
                <div className="flex items-center gap-4 text-sm text-gray-500 mb-6">
                  <div className="flex items-center gap-1">
                    <User className="w-4 h-4" /> {briefs[0].author}
                  </div>
                  <div className="flex items-center gap-1">
                    <Clock className="w-4 h-4" /> {briefs[0].readTime}
                  </div>
                  <span>{briefs[0].date}</span>
                </div>
                <Link
                  to={`/briefs/${briefs[0].id}`}
          ...
          {/* All Briefs */}
          <h3 className="font-heading font-semibold text-2xl text-steel mb-8">All Briefs</h3>
            ...
            {briefs.map((brief, i) => (
              ...
              >
                ...
                >
                  ...
                  <p className="text-gray-600 text-sm leading-relaxed mb-4 line-clamp-3">{brief.excerpt}</p>
                  <div className="flex items-center gap-3 text-xs text-gray-400">
                    <span>{brief.date}</span>
                    <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {brief.readTime}</span>
                  </div>
                </Link>
              </motion.div>
            ))}
